    ...
    def store_chunks_for_doc(self, doc: ProcessedDocument):
        ...
        for chunk in doc.chunks:
            ...
            if chunk.nodes is not None:
                ...
                # Enforce the ontology (if any): drop entities / relationships
                # whose type is outside the allowed schema. The extractor is
                # already prompted with the ontology, but this is the hard
                # guarantee that nothing off-schema lands in Neo4j.
                nodes = list(chunk.nodes)
                relationships = list(chunk.relationships) if chunk.relationships else []
                allowed_labels = getattr(self, "allowed_labels", None)
                allowed_rels = getattr(self, "allowed_relationships", None)

                if allowed_labels:
                    nodes = [n for n in nodes if n.type in allowed_labels]
                    kept_ids = {n.id for n in nodes}
                    relationships = [
                        r for r in relationships
                        if getattr(r.source, "id", r.source) in kept_ids
                        and getattr(r.target, "id", r.target) in kept_ids
                    ]
                if allowed_rels:
                    relationships = [r for r in relationships if r.type in allowed_rels]
